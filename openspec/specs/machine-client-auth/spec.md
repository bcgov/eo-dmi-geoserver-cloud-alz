# Spec: Machine-Client Authentication

## Purpose

Machine/API clients (non-interactive callers of GeoServer's OWS services) authenticate
via GeoServer's `auth-key` extension. Each machine client is a synthetic, per-key
identity provisioned independently — `infra/stack/` (Terraform) and
`infra/scripts/configure-geoserver-security.sh` own this provisioning. Authorization for
what a given machine-client identity can access is a separate concern, owned entirely by
`geo-server-app-config/catalog/acl_rules.yaml` (see the `catalog` spec's username-scoped
ACL rule requirements) — this capability never grants access, only identity.

## Requirements

### Requirement: Machine-client identities are provisioned independently per username
The system MUST provision one GeoServer `authkey`-authenticated identity per entry in
...
- THEN a new Key Vault secret `geoserver-machine-authkey-<username>` is generated (if
  absent) and `configure-geoserver-security.sh` creates a corresponding GeoServer user
  and `authkeys.properties` entry, without modifying any other username's secret, user,
  or key
...
The shared `authKey` filter MUST be wired only into the `default` (OWS: WMS/WFS/WCS/WPS)
and `gwc` (tile cache) filter chains, and MUST NOT be present in the `web` (admin GUI) or
...
`null_resource.secret_geoserver_machine_authkey` MUST check for an existing secret named
