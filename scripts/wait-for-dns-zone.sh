...
main() {
	...
	while true; do
		...
		if [[ "$SECONDS" -ge "$timeout_seconds" ]]; then
			if [[ -n "$private_endpoint_name" ]]; then
				echo "WARN: Timed out waiting for private endpoint DNS zone group(s) (rg='$resource_group', endpoint='$private_endpoint_name') after $timeout. BC Gov ALZ policy attaches zone groups asynchronously — DNS will resolve once policy evaluation completes." >&2
			elif [[ -n "$zone_id" ]]; then
				echo "WARN: Timed out waiting for DNS zone (id='$zone_id') after $timeout." >&2
			else
				echo "WARN: Timed out waiting for DNS zone (type='$zone_type', rg='$resource_group', name='$zone_name') after $timeout." >&2
			fi
			return 0
		fi
